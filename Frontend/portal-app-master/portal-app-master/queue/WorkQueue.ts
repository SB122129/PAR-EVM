...
import type { DatabaseService, QueuedTaskRecord } from '@/services/DatabaseService';
...
function deserializeValue(v: string): any {
  return JSON.parse(v, (_, value) => {
    if (
      ...
      // Keep serialized calendar values as strings in environments where parser is unavailable.
      return value;
    } else {
      return value;
    }
...
// Helper function to check if a value looks like a calendar-like object.
function isCalendarInterface(
  value: unknown
): value is {
  nextOccurrence: (...args: unknown[]) => unknown;
  toCalendarString: (...args: unknown[]) => string;
  toHumanReadable: (...args: unknown[]) => string;
} {
  return (
    typeof value === 'object' &&
    value !== null &&
