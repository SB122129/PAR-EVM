...
export default function VaultScreen() {
  ...
  const [lastSuccessfulDemoProof, setLastSuccessfulDemoProof] = useState<DemoProofSnapshot | null>(null);
  ...
  const [walletConnectStatus, setWalletConnectStatus] = useState<WalletConnectEvmStatus | null>(null);
    ...
    try {
      for (const check of failedChecks) {
        try {
          ...
          if (check.label === 'AI recommendation available' || check.label === 'Attestation verified') {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content}>
        <ThemedText type="title">EVM Vault</ThemedText>
        <ThemedText style={styles.subtitle}>
          Polkadot Hub EVM track integrated directly into the mobile app.
        </ThemedText>
        ...
        <ThemedView style={styles.card}>
          ...
          <View style={styles.controlGrid}>
            <TouchableOpacity
              style={[styles.controlButton, (isRunningDemoScript || isSubmittingTx) && styles.disabledButton]}
            ...
            <TouchableOpacity
              style={[styles.controlButton, isAutoFixing && styles.disabledButton]}
              onPress={onRunAutoFix}
              disabled={isAutoFixing}
            >
              <ThemedText style={styles.actionButtonText}>Auto Fix</ThemedText>
            </TouchableOpacity>
            <TouchableOpacity style={styles.controlButton} onPress={onCopyDemoReport}>
              <ThemedText style={styles.actionButtonText}>Copy</ThemedText>
            </TouchableOpacity>
            <TouchableOpacity style={styles.controlButton} onPress={onSaveDemoReportJson}>
              <ThemedText style={styles.actionButtonText}>Save JSON</ThemedText>
            </TouchableOpacity>
            <TouchableOpacity style={styles.controlButton} onPress={onShareDemoReport}>
              <ThemedText style={styles.actionButtonText}>Share</ThemedText>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.controlButton, !lastSuccessfulDemoProof && styles.disabledButton]}
        ...
        <ThemedView style={styles.card}>
          ...
          <ThemedText style={[styles.readinessVerdict, { color: fraudStatusColor }]}>
            Status: {overview.fraudAssessment.status.toUpperCase()} ({overview.fraudAssessment.score}/100)
          </ThemedText>
        ...
        {signerSetup && showAdvancedSections ? (
          <ThemedView style={styles.card}>
            ...
            <ThemedText>
              WalletConnect Project ID: {signerSetup.walletConnectProjectIdConfigured ? 'Configured' : 'Missing'}
            </ThemedText>
            <ThemedText style={styles.muted}>ID: {signerSetup.walletConnectProjectIdPreview}</ThemedText>
            <ThemedText>
              External Signer Registered: {signerSetup.externalSignerRegistered ? 'Yes' : 'No'}
            </ThemedText>
            <ThemedText>
              Unlocked RPC Fallback: {signerSetup.unlockedRpcFallbackEnabled ? 'Enabled' : 'Disabled'}
            </ThemedText>
            <ThemedText>
            ...
            {walletConnectStatus ? (
              <View style={styles.txStatusBox}>
                <ThemedText>WC Initialized: {walletConnectStatus.initialized ? 'Yes' : 'No'}</ThemedText>
                <ThemedText>WC Connected: {walletConnectStatus.connected ? 'Yes' : 'No'}</ThemedText>
                <ThemedText>
                  Wallet Address:{' '}
                ...
                <ThemedText>
                  Wallet Chain ID:{' '}
                  {walletConnectStatus.chainId !== null ? walletConnectStatus.chainId : 'Unavailable'}
                </ThemedText>
        ...
        {showAdvancedSections ? (
          <ThemedView style={styles.card}>
            ...
            <ThemedText>User Deposit: ${overview.snapshot.userDepositUsd.toLocaleString()}</ThemedText>
        ...
        {showAdvancedSections ? (
          ...
          <ThemedText>
            Rebalance Cooldown: {overview.policy.rebalanceCooldownHours}h
          </ThemedText>
          <ThemedText>
        ...
        {showAdvancedSections ? (
          ...
          {overview.rebalanceMetrics.map(metric => (
            <View key={metric.id} style={styles.txStatusBox}>
              ...
              <ThemedText style={styles.muted}>
                Before: risk {metric.before.riskScore}/100, APY {metric.before.apyPercent}%, stable {metric.before.stableExposurePercent}%
              </ThemedText>
              <ThemedText style={styles.muted}>
                After: risk {metric.after.riskScore}/100, APY {metric.after.apyPercent}%, stable {metric.after.stableExposurePercent}%
              </ThemedText>
              <ThemedText>
        ...
        {showAdvancedSections ? (
          ...
          <TouchableOpacity
            style={[styles.actionButton, (isRunningDemoScript || isSubmittingTx) && styles.disabledButton]}
            onPress={onRunDemoScript}
            disabled={isRunningDemoScript || isSubmittingTx}
          >
          ...
          {demoRunResult ? (
            <View style={styles.txStatusBox}>
              <ThemedText>
                Result: {demoRunResult.success ? 'Success' : 'Failed'}
              </ThemedText>
              <ThemedText style={styles.muted}>
        ...
        {lastSuccessfulDemoProof ? (
          <ThemedView style={styles.card}>
            ...
            <ThemedText style={styles.muted}>
              Demo Finished: {new Date(lastSuccessfulDemoProof.demoRunResult.finishedAtIso).toLocaleString()}
            </ThemedText>
            <ThemedText style={styles.muted}>
              Chain: {lastSuccessfulDemoProof.chain.name} ({lastSuccessfulDemoProof.chain.source})
            </ThemedText>
            <ThemedText style={styles.muted}>
              Recommendation: {lastSuccessfulDemoProof.recommendation.source} / attestation {lastSuccessfulDemoProof.recommendation.attestation.status}
            </ThemedText>
            <ThemedText style={styles.muted}>
          ...
          {lastExecution ? (
            <View style={styles.txStatusBox}>
              ...
              {lastExecution.txHash ? (
                <ThemedText style={styles.muted}>Tx: {lastExecution.txHash.slice(0, 18)}...</ThemedText>
              ) : null}
              {lastExecution.explorerUrl ? (
                <ThemedText style={styles.muted}>Explorer: {lastExecution.explorerUrl}</ThemedText>
              ) : null}
              {lastExecution.error ? <ThemedText style={styles.errorText}>{lastExecution.error}</ThemedText> : null}
              <ThemedText style={styles.muted}>
        ...
        {showAdvancedSections ? (
          ...
          <ThemedText style={styles.muted}>Hash: {overview.recommendation.hash.slice(0, 18)}...</ThemedText>
          <ThemedText style={styles.muted}>
          ...
          {overview.recommendation.attestation.error ? (
            <ThemedText style={styles.errorText}>{overview.recommendation.attestation.error}</ThemedText>
          ) : null}
          <ThemedText style={styles.muted}>
            Updated: {new Date(overview.recommendation.createdAtIso).toLocaleString()}
          </ThemedText>
          <ThemedText style={styles.muted}>
            Checked: {new Date(overview.recommendation.attestation.lastCheckedIso).toLocaleString()}
          </ThemedText>
          </ThemedView>
        ) : null}
