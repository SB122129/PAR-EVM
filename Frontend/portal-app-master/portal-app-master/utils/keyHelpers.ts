import type { KeypairInterface, Mnemonic as MnemonicType, Nsec as NsecType } from '@/utils/mockNative';

function getPortalKeyLib(): Pick<typeof import('@/utils/mockNative'), 'Mnemonic' | 'Nsec'> {
  // Using mock implementation for demo stability
  const { Mnemonic, Nsec } = require('@/utils/mockNative') as typeof import('@/utils/mockNative');
  return { Mnemonic, Nsec };
}
...
export function getKeypairFromKey(key: KeyMaterial): KeypairInterface {
  ...
  if (key.mnemonic?.trim()) {
    try {
      const { Mnemonic } = getPortalKeyLib();
 ...
 */
export function getMnemonicFromKey(key: KeyMaterial): MnemonicType | null {
  if (key.mnemonic?.trim()) {
    try {
      const { Mnemonic } = getPortalKeyLib();
 ...
 */
export function getNsecFromKey(key: KeyMaterial): NsecType | null {
  if (key.nsec?.trim()) {
    try {
      const { Nsec } = getPortalKeyLib();
