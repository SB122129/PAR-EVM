...
export class EvmVaultService {
  ...
  private readonly attestationRegistryAddress =
    env.EXPO_PUBLIC_EVM_ATTESTATION_REGISTRY_ADDRESS || '';
  ...
  private readonly fraudBlockScore = Number.parseInt(
    env.EXPO_PUBLIC_EVM_FRAUD_BLOCK_SCORE || '60',
    10
  );
  ...
  async runDemoScenario(onProgress?: (state: EvmTxExecution) => void): Promise<EvmDemoRunResult> {
    const startedAtIso = new Date().toISOString();
    const steps: EvmDemoStep[] = [];
    let lastIntent: EvmTxIntent | null = null;
    let lastExecution: EvmTxExecution | null = null;

    const pushStep = (label: string, status: EvmDemoStep['status'], detail: string): void => {
      steps.push({
  ...
  buildDemoReportText(input: EvmDemoReportInput): string {
    const { overview, demoRunResult, lastIntent, lastExecution, walletConnected, walletAddress } =
      input;
    ...
    if (lastIntent) {
      lines.push(
        `Last intent: ${lastIntent.type} $${lastIntent.amountUsd} mode=${lastIntent.mode}`
      );
    } else {
