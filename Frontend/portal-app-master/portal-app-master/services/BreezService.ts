...
import { Nsec } from '@/utils/mockNative';
import {
...
export class BreezService implements Wallet {
  ...
  private async init(nsec: string) {
    ...
    if (!apiKey) {
      throw new Error(
        'Missing EXPO_PUBLIC_BREEZ_API_KEY. Set it in .env to enable Breez wallet.'
      );
    }
  ...
  async receivePayment(amountSats: bigint, description?: string): Promise<string> {
    const response = await this.client.receivePayment({
      paymentMethod: new ReceivePaymentMethod({
  ...
  async sendPayment(paymentRequest: string, amountSats: bigint): Promise<string> {
    ...
    const sendOptions: SendPaymentOptions | undefined = new SendPaymentOptions.Bolt11Invoice({
      preferSpark: false,
  ...
  async sendPaymentWithPrepareResponse(
    _prepareResponse: PrepareSendPaymentResponse
  ): Promise<string> {
    const sendOptions: SendPaymentOptions | undefined = new SendPaymentOptions.Bolt11Invoice({
      preferSpark: false,
      completionTimeoutSecs: 30,
