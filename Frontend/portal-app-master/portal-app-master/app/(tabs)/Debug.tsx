  ...
  const validateQRCode = (data: string): { isValid: boolean; error?: string } => {
    if (isNWCMode) {
      ...
      try {
        ...
        return {
          isValid: false,
          error: 'Invalid QR code. Please enter a valid PAR authentication QR code.',
        };
      }
    }
    return { isValid: true };
  ...
  return (
    <ThemedView style={[styles.section, { backgroundColor: cardBackgroundColor }]}>
      ...
      <View style={styles.settingRow}>
        <View style={styles.settingInfo}>
          ...
          <ThemedText style={[styles.settingSubtext, { color: secondaryTextColor }]}>
            {isNWCMode ? 'Test Nostr Wallet Connect (NWC) URLs' : 'Test PAR authentication URLs'}
          </ThemedText>
        </View>
        <Switch
          value={isNWCMode}
      ...
      <View style={styles.settingColumn}>
        ...
        <ThemedText style={[styles.settingSubtext, { color: secondaryTextColor }]}>
          {isNWCMode ? 'Enter a nostr+walletconnect:// URL' : 'Enter an authentication URL'}
        </ThemedText>
        ...
        <TextInput
          ...
          placeholder={isNWCMode ? 'nostr+walletconnect://...' : 'Paste authentication URL...'}
...
// Logger callback for Rust logging
const rustLogCallback: LogCallback = (entry: LogEntry) => {
  const message = `[mock-native] ${entry.message}`;
  switch (entry.level) {
    case LogLevel.Debug:
...
function RustLogger({
  ...
  return (
    <ThemedView style={[styles.section, { backgroundColor: cardBackgroundColor }]}>
      ...
      <ThemedText style={[styles.description, { color: secondaryTextColor }]}>
        Initialize Rust logging from the PAR app library. Logs will appear in the console.
      </ThemedText>
      ...
      <View style={styles.settingColumn}>
        ...
        <View style={styles.logLevelContainer}>
          {logLevelOptions.map(option => (
            <TouchableOpacity
              key={`rust-log-level-${option.label.toLowerCase()}`}
              style={[
