...
export default function ErrorScreen() {
  ...
  // Helper to convert nsec or npub to hex
  const keyToHex = (key: string): string => {
    try {
  ...
  const publicKeyHexValue = useMemo(() => {
  ...
  const emailBody =
    ...
    `Public Key (hex): ${publicKeyHexValue ? `${publicKeyHexValue.substring(0, 20)}...${publicKeyHexValue.substring(publicKeyHexValue.length - 10)}` : 'N/A'}\n` +
  ...
  const mailto = `mailto:${supportEmail}?subject=PAR App - Unrecoverable Error&body=${encodeURIComponent(emailBody)}`;
