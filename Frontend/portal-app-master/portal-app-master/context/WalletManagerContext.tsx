...
export const WalletManagerContextProvider: React.FC<WalletManagerContextProviderProps> = ({
  ...
  const setupBreezEventListener = useCallback(
    (breezWallet: { addEventListener: (arg: unknown) => Promise<unknown> }) => {
      const handler = async (event: any) => {
        const paymentData = event?.inner?.payment;
        if (!paymentData) return;

        const { amount, id, status: pStatus, details } = paymentData ?? {};
        const amountInSats = Number(amount ?? 0);
        ...
        const eventType =
          pStatus === 'pending' || pStatus === 'Pending'
            ? 'pending'
            : pStatus === 'completed' || pStatus === 'Completed'
              ? 'succeeded'
              : 'failed';
        ...
        try {
          ...
          const invoice = details?.inner?.invoice ?? null;
    ...
    async <T extends WalletType>(walletType: T): Promise<WalletTypeMap[T]> => {
      ...
      switch (walletType) {
        case WALLET_TYPE.BREEZ:
          // Expo Go cannot load custom native modules (like Breez SDK).
          // Only attempt Breez when not running inside Expo Go and when configured.
          if (Constants.appOwnership === 'expo') {
            throw new Error('Breez wallet is not supported in Expo Go. Use a dev build.');
          }
          {
            const { BreezService } = await import('@/services/BreezService');
            instance = (await BreezService.create(
              nsecToUse!,
              onStatusChange(walletType)
            )) as WalletTypeMap[T];
            // Setup event listener for Breez wallet
            setupBreezEventListener(instance as any);
          }
          break;
  ...
  const initializeWalletManager = useCallback(async () => {
    try {
      const hasBreezApiKey = Boolean(process.env.EXPO_PUBLIC_BREEZ_API_KEY);

      // Initialize Breez wallet at startup (only if configured)
      if (hasBreezApiKey) {
      ...
      if (stored) {
        ...
        if (walletType === WALLET_TYPE.BREEZ && !hasBreezApiKey) {
          // Breez isn't configured; keep app usable instead of failing on startup
          setIsWalletManagerInitialized(true);
          return;
        }
        await switchActiveWallet(walletType);
      } else {
        // Default to Breez only when configured
        if (hasBreezApiKey) {
